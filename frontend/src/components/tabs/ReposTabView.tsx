...
export function ReposTabView({
  ...
  if (loading) {
    return (
      <section className="grid grid-cols-1 gap-4 sm:col-span-2 xl:col-span-3 xl:grid-cols-3">
    ...
    <section className="grid grid-cols-1 gap-4 sm:col-span-2 xl:col-span-3 xl:grid-cols-3 sm:gap-6">
      <Card title="Repository Workspace">
        <div className="rounded-xl border border-white/15 bg-slate-900/35 px-4 py-4">
          <p className="text-xs uppercase tracking-[0.12em] text-slate-400">Active Repository</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <Stat label="Health Score" value={`${repoHealthScore}/100`} tone={repoHealthScore >= 70 ? 'good' : 'warn'} />
          <Stat label="Commits / Week" value={commitsPerWeek.toFixed(1)} />
          <Stat label="PR Merge Avg" value={averageReviewHours === null ? 'N/A' : `${averageReviewHours}h`} />
          <Stat label="Issue Closure" value={`${issueClosureRate}%`} tone={issueClosureRate >= 65 ? 'good' : 'warn'} />
        </div>

        <div className="mt-4 grid grid-cols-3 gap-2 text-xs text-slate-300">
          <span className="rounded-md border border-white/10 bg-slate-900/40 px-2 py-1.5">PRs {prs.length}</span>
          <span className="rounded-md border border-white/10 bg-slate-900/40 px-2 py-1.5">Open PRs {openPrCount}</span>
          <span className="rounded-md border border-white/10 bg-slate-900/40 px-2 py-1.5">Merged {mergedPrCount}</span>
          <span className="rounded-md border border-white/10 bg-slate-900/40 px-2 py-1.5">Issues {issues.length}</span>
          <span className="rounded-md border border-white/10 bg-slate-900/40 px-2 py-1.5">Open {openIssueCount}</span>
          <span className="rounded-md border border-white/10 bg-slate-900/40 px-2 py-1.5">Closed {closedIssueCount}</span>
        </div>
      </Card>
